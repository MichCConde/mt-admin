# Identical logic to original eod.py router
                    ...
                    if reports:
                        ...
                        missing.append({
                            **va,
                            ...
                            "missing_type":   "clock_in_only" if not clocked_in else "eod_only",
                            "missing_reason": (
                                "No clock-in and no EOD"
                                if not clocked_in
                                else "Clocked in but no EOD submitted"
                            ),
                        })
                    continue
                    ...
                    if reports:
                        ...
                        missing.append({
                            **va,
                            ...
                            "missing_type":   "clock_in_only" if not clocked_in else "eod_only",
                            "missing_reason": (
                                f"No clock-in and no EOD for: {contract['client_name']}"
                                if not clocked_in
                                else f"Clocked in but no EOD for: {contract['client_name']}"
                            ),
                        })
